...
def get_uncompressed_size_buffer(data: bytes) -> int:
    ...
    return int.from_bytes(data[:4], "little") if len(data) >= 4 else -1
...
def decompress_buffer(data: bytes) -> bytes:
    """Decode an Ultima VI LZW stream, including its four-byte size header."""
    if not is_valid_lzw_buffer(data):
        raise FormatError("lzw", "not a valid Ultima VI LZW buffer")
    ...
    def read_code() -> int:
        nonlocal bit_offset
        if bit_offset + width > len(source) * 8:
            raise FormatError("lzw", "truncated code stream", bit_offset // 8 + 4)
        byte = bit_offset // 8
    ...
    while True:
        ...
        if code == 0x100:
            ...
            code = read_code()
            if code > 0xFF:
                raise FormatError("lzw", "invalid first code after dictionary reset")
            entry = bytes((code,))
            output.extend(entry)
            previous = entry
            continue
        ...
        if code < 0x100:
            ...
            raise FormatError("lzw", f"invalid LZW code 0x{code:x}")
        ...
        if len(output) > expected:
            raise FormatError("lzw", "stream exceeds its declared output size")

    if len(output) != expected:
        raise FormatError("lzw", f"output is {len(output)} bytes; expected {expected}")
    return bytes(output)

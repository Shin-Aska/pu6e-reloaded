...
def parse_renderer_mode(value: str | None) -> RendererMode:
    if value is None:
...
def read_renderer_mode(config_path: Path) -> RendererMode:
    ...
    try:
        _ = configuration.read(config_path, encoding="utf-8")
    except configparser.Error:
        return RendererMode.OPENGL
    return parse_renderer_mode(configuration.get("launcher", "renderer", fallback=None))

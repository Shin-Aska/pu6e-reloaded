...
from ui.launcher.stage import WorldStage
from ui.launcher.style import launcher_stylesheet
from ui.profile.dialog import GameConfigurationDialog
from ui.profile.models import GAMES, GameProfile
...
from ui.shared.icons import action_icon
...
class LauncherWindow(QWidget):
    def __init__(
        ...
        *,
        launch_editor: Callable[[Path], EditorController],
        restart_application: Callable[[], bool],
    ) -> None:
        super().__init__()
        self.store = store
    ...
    def configure_launcher(self) -> None:
        ...
        vulkan_gpu = self.settings.vulkan_gpu
        dialog = LauncherSettingsDialog(self.settings, self)
        if dialog.exec() != dialog.DialogCode.Accepted:
            return
        graphics_changed = (
            self.settings.renderer is not renderer
            or self.settings.vulkan_gpu != vulkan_gpu
        )
        from ui.settings.dialog import offer_renderer_restart
    ...
    def launch_game(self, game: str) -> None:
        ...
        try:
            ...
            controller = self._launch_editor(self.store.config_path)

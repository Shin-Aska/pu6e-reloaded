...
def test_mesa_runtime_path_remains_project_relative_after_package_move() -> None:
    from ui.runtime.windows.vulkan import mesa_directory

    assert mesa_directory() == _PROJECT / "build" / "mesa" / "runtime"
...
def test_application_import_keeps_opengl_unbound_until_renderer_selection() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-c", "import sys; import ui.app.bootstrap; "
...
def test_windows_vulkan_draws_with_the_bundled_runtime(software: bool) -> None:
    ...
    if not software:
        from ui.runtime.vulkan import VulkanDeviceKind, list_vulkan_devices
    ...
    script = f"""
from ui.runtime.renderer import configure_renderer, RendererMode
...
def test_renderer_probe_reports_verified_pixels(tmp_path: Path) -> None:
    ...
    from ui.runtime.renderer import RendererMode, _set_renderer_environment
    ...
    result = subprocess.run(
        [sys.executable, "-m", "ui.runtime.probe", str(report)],

...
core can load, inspect, edit, and save a world without importing Qt or OpenGL.
...
| --- | --- |
| `pu6e_core.models` | Game types, coordinates, objects and NPCs, terrain, immutable asset catalogs, and mutable world state. |
| `pu6e_core.formats` | Game resource manifests, DOS path resolution, compression, and binary decoding and encoding. |
| `pu6e_core.services` | Loading complete worlds, querying and editing world state, and saving changed game files. |
| `pu6e_qt` | Launcher, configuration, controller signals, undo commands, workbench, and inspection tools. |
| `pu6e_qt.rendering` | Camera geometry, rendering options, OpenGL batches, animation, and GPU resources. |
| `pu6e.py` | Application entry point, also exposed as the `pu6e` console command. |

Place code with the concept it implements: coordinate operations belong in
`models.coordinates`, file encodings in `formats`, and world operations in
`services.editor`. Small functions stay beside their callers until they have a
...
Services depend on models and formats. Formats depend on models where a
decoded value needs a domain type. Models do not import services. The Qt layer
depends on the core; the core does not depend on presentation code.
...
core data and does not load game files.

## Distribution and provenance

Setuptools discovers `pu6e_core` and `pu6e_qt` recursively. The frozen Linux
and Windows builds collect the core and rendering subpackages alongside
OpenGL's dynamically selected platform modules. The Windows build also
includes its existing OpenGL integration and bundled Mesa runtime.
